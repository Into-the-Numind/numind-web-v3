...
test('settings Feishu card is status-only and never starts authorization', async ({ page }) => {
    ...
    await route.fulfill({
      ...
      body: JSON.stringify({
        ...
        data: {
          ...
          active_action: {
            operation_id: 'settings-connect-op',
  ...
  await page.route('**/v1/feishu/connect', async (route) => {
    ...
    await route.fulfill({
      status: 500,
      contentType: 'application/json',
      body: JSON.stringify({ code: 500, message: 'settings must not authorize' })
    })
  })
  ...
  await expect(page.getByTestId('feishu-connection-empty')).toContainText('需要重新授权')
  await expect(page.getByTestId('feishu-reauthorize')).toHaveCount(0)
  await expect(page.getByTestId('feishu-manual-action')).toHaveCount(0)
  await expect(page.getByTestId('feishu-open-action')).toHaveCount(0)
  await expect(page.getByText('settings-opaque')).toHaveCount(0)
